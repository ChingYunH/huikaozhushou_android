...
public class ChemistryReview extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_chemistry_review);
		getLink();
		initialize();
	}
	...
	private void initBar() {
		// TODO Auto-generated method stub
		total = MyData.ReviewNum[MyData.chemistryreview] - 1;
		bar.setMax(total);
		bar.setProgress(0);
	}
	...
	private void pageRefresh(int pageID) {
		...
		else bDescript.setText(MyData.Review[MyData.chemistryreview][now].getDescription());
		if(!answer_always) bAnswer.setText(R.string.clickToShow);
		else bAnswer.setText(MyData.Review[MyData.chemistryreview][now].getAnswer());
	...
	class CheckBox_Ans_Change implements OnCheckedChangeListener {
		...
		public void onCheckedChanged(CompoundButton buttonView,	boolean isChecked) {
			...
			if( isChecked ) {
				answer_always = true;
				answer_status = true;
				bAnswer.setText(MyData.Review[MyData.chemistryreview][now].getAnswer());
			}
			else {
				answer_always = false;
			}
	...
	class CheckBox_Des_Change implements OnCheckedChangeListener {
		...
		public void onCheckedChanged(CompoundButton buttonView,	boolean isChecked) {
			...
			if( isChecked ) {
				...
				bDescript.setText(MyData.Review[MyData.chemistryreview][now].getDescription());
			} 
			else {
